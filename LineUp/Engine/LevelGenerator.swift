...
struct IntroGame {
    ...
    var shapeName: String {
        LevelGenerator.previewName(levelType: levelType, dotCount: dotCount, game: 1)
    }
}
...
enum LevelGenerator {
    ...
    /// Convenience for backward-compatible call sites.
    static func shapeName(dotCount: Int, isCurve: Bool) -> String {
        if isCurve { return dotCount == 2 ? "Arc" : "Curve·\(dotCount)" }
